...
对照训练的目的是确认数据清洗是否真的有效。这里同时展示整体指标和每个类别的指标，避免只看整体平均值而漏掉 RC柱 的短板。

| 数据集版本 | 整体 P/R/mAP50/mAP50-95 | 天井 P/R/mAP50 | 壁类 P/R/mAP50 | RC柱 P/R/mAP50 | 结论 |
|---|---|---|---|---|---|
| full | 0.720 / 0.591 / 0.695 / 0.543 | 0.837 / 0.707 / 0.829 | 0.774 / 0.626 / 0.753 | 0.549 / 0.440 / 0.502 | 保留更多数据，但 RC柱 召回明显偏低 |
| cleaned | 0.699 / 0.673 / 0.721 / 0.554 | 0.851 / 0.760 / 0.858 | 0.661 / 0.730 / 0.760 | 0.586 / 0.529 / 0.545 | Recall 和 mAP 提升，作为后续调优基础更稳 |
...
结论：数据清洗有效。`cleaned` 相比 `full`，整体 Recall 从 `0.591` 提升到 `0.673`，mAP50 从 `0.695` 提升到 `0.721`；RC柱 的 Recall 也从 `0.440` 提升到 `0.529`。这说明清洗后模型更能找回真实建筑区域，后续 RC柱 调优以 `cleaned` 为基础继续推进。
...
| 版本 | 改进内容 | 整体 P/R/mAP50 | 天井 P/R | 壁类 P/R | RC柱 P/R/mAP50 | 端到端 RC柱 命中 |
|---|---|---|---|---|---|---|
| baseline cleaned | 清洗数据基础模型 | 0.627 / 0.653 / 0.694 | 0.757 / 0.760 | 0.583 / 0.726 | 0.542 / 0.474 / 0.547 | 13/20 |
| B image-weights | 提高难样本训练权重 | 0.680 / 0.708 / 0.745 | 0.758 / 0.813 | 0.663 / 0.749 | 0.618 / 0.561 / 0.623 | 仍需进一步改善 |
| C RC oversample | 增加 RC柱 样本出现比例 | 0.641 / 0.684 / 0.733 | 0.748 / 0.813 | 0.661 / 0.695 | 0.514 / 0.544 / 0.609 | RC柱 召回有改善但精度不足 |
| D800 | image-weights + RC柱 适度扩增 | 0.711 / 0.670 / 0.731 | 0.775 / 0.740 | 0.710 / 0.691 | 0.648 / 0.579 / 0.620 | 综合不如 D900 |
| D900 | image-weights + RC柱 扩增到 900 | 0.753 / 0.673 / 0.761 | 0.832 / 0.748 | 0.759 / 0.676 | 0.666 / 0.596 / 0.664 | 16/20 |
| 当前最好版本 | D900 策略 + 追加数据后继续训练 | 0.843 / 0.841 / 0.888 | 0.854 / 0.880 | 0.786 / 0.791 | 0.888 / 0.853 / 0.905 | 后续继续扩大端到端验证 |
