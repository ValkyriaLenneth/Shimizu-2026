...
ただし、現在の最終表示画像では、複数候補を持つ代表枠について「この 1 つの枠に RC柱 候補と壁類候補が含まれている」という候補一覧までは描画していない。処理データとしては複数候補を保持しているが、今回の可視化では代表枠のみを表示している。

今後の改善点：

- フロントエンドへ渡すデータでは、代表枠 1 つに対して複数の部材候補を保持する。
- 可視化上も、代表枠に紐づく候補一覧を表示できるようにする。
- 斜めの天井や柱のように矩形だけでは表現しにくいケースでは、代表枠と候補情報を分けて確認できる形が望ましい。
