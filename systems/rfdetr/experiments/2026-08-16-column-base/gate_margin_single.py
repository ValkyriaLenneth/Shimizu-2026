#!/usr/bin/env python3
"""Re-tune the member-box margin under the single-box gate.
...
def main():
    ...
    print(f"{'外扩':>8} {'P':>7} {'R':>7} {'B':>6} {'C':>6} {'D':>6} {'发火':>6} {'箱/张':>7} {'阈值':>18}")
    rows = []
    for mg in MARGINS:
        # The margin was set to 0.10 to absorb router localisation error when the
        # gate used the union of every box. The single box is a much smaller
        # region, so how much slack it needs is an open question again.
        RG.MARGIN = mg
        ...
        if not best:
            print(f"{mg:8.2f}   无四项达标点"); continue
        p_, r_, per, combo, fire, _, bpi = best
        print(f"{mg:8.2f} {p_:7.3f} {r_:7.3f} {per[0]:6.3f} {per[1]:6.3f} {per[2]:6.3f} "
              f"{fire:5.0%} {bpi:7.2f} {str(combo):>18}")
        rows.append({"margin": mg, "precision": p_, "recall": r_, "per": per,
                     "thr": combo, "fire": fire, "bpi": bpi})
    cur = [r for r in rows if abs(r["margin"] - 0.10) < 1e-9]
    if cur and rows:
        # Ties on false alarms are broken by precision, since the sweep's own
        # objective cannot separate them and precision is the reported figure.
        b = min(rows, key=lambda r: (round(r["bpi"], 4), -r["precision"]))
        print(f"\n现配置 外扩 0.10: {cur[0]['bpi']:.2f} 箱/张,P {cur[0]['precision']:.3f}")
        print(f"最优 外扩 {b['margin']:.2f}: {b['bpi']:.2f} 箱/张,P {b['precision']:.3f}")
        print("-> " + ("与现配置相同,无需改动" if abs(b["margin"] - 0.10) < 1e-9
                       else f"外扩 {b['margin']:.2f} 更优,须再经留出验证"))
    Path("/workspace/exp_cb/gate_margin_single.json").write_text(
