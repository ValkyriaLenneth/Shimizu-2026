#!/usr/bin/env python3
"""Copy-paste positives with the target resampled to the crop's own scale.

The first build of this arm pasted damage crops into sound photographs without
reconciling their resolutions, and the two populations differ: damaged images run
0.31 megapixels at the median against the client's sound photographs at 3.15.
Measured per paste rather than per corpus, 63% of pastes enlarged the crop by
more than 4x in area before the composite was resized to the model's input,
leaving a blurred patch visibly distinct from the sharp scene around it.

That artefact is learnable, and the evidence says it was learned: the baseline
scored pasted damage at 0.110 against 0.609 for real damage, and the first build
emitted 22% more confident boxes while precision fell 0.141 -- firing more often
and being right less, which is what "detect the blurry patch" looks like from
outside.

This build resamples the sound target down to the crop's own pixel scale before
pasting, so the composite carries one level of detail throughout. The target
loses resolution, but the corpus the model is evaluated on sits at that scale
anyway, which makes matching it the more faithful choice rather than a
compromise.

Provenance stays clean by construction.
...
CP = Path("/workspace/Shimizu-2026/data/rfdetr_column_base_cv5_cp2")
...
def main():
    ...
    for f in range(5):
        ...
        for p in sorted((src / "train" / "images").iterdir()):
            ...
            with Image.open(p) as ih:
                src_mp = ih.size[0] * ih.size[1] / 1e6
            for c, x, y, w, h in boxes:
                crops.append((p, c, int(x), int(y), int(w), int(h), src_mp))
        targets = [p for p in sound if p.name not in sfolds[f] and sites[p.name]]
        made = 0
        for p in targets:
            for v in range(VARIANTS):
                picks = [rng.choice(crops) for _ in range(PER_IMAGE)] if crops else []
                if not picks:
                    continue
                with Image.open(p) as h:
                    im = h.convert("RGB")
                # Bring the target down to the crops' scale, so the pasted region
                # and its surroundings carry the same detail through the model's
                # resize. Site coordinates move with it.
                tgt_mp = im.size[0] * im.size[1] / 1e6
                want_mp = float(np.median([q[6] for q in picks]))
                k_img = (want_mp / tgt_mp) ** 0.5 if 0 < want_mp < tgt_mp else 1.0
                if k_img < 1.0:
                    im = im.resize((max(64, int(im.width * k_img)),
                                    max(64, int(im.height * k_img))), Image.LANCZOS)
                sx1, sy1, sx2, sy2 = (int(val * k_img) for val in sites[p.name])
                lines = []
                for cp_, c, x, y, w, h_, _mp in picks:
    ...
    print(f"\n评测集与对照组逐张一致: {same}")
    Path("/workspace/exp_cb/cv_cp2_summary.json").write_text(
