...
for combo in itertools.product(range(len(G)), repeat=NC):
    ...
    # All three grades are free to sit where they measure, since the delivery
    # writes each grade's own figure. Reliability is then how often a resample
    # still meets all three of those figures at once, which is the promise as the
    # client would actually read it.
    if min(per) < 0.60: continue
    ...
    for idx in idxs:
        ok=True
        for c in range(NC):
            tc=tp[idx,c,combo[c]].sum(); mc=fn[idx,c,combo[c]].sum()
            if (tc/max(tc+mc,1)) < per[c]: ok=False; break
        hold += ok
    rows.append({"thr":[G[i] for i in combo],"P":P,"B":per[0],"C":per[1],"D":per[2],
                 "rel":hold/NBOOT,"bpi":bpi(combo)})

print(f"共 {len(rows)} 个候选点(三级均 >= 0.60,承诺=各自实测值)\n")
...
Path("/workspace/exp_cb/iso_all.json").write_text(json.dumps(rows,indent=2))
