...
CONFS = ["max", "avg", "box_and_model_avg"]
...
print(f"{'conf_type':20s} {'P':>7} {'R':>7} {'B':>6} {'C':>6} {'D':>6} {'发火':>6} {'箱/张':>7}")
rows=[]
for ct in CONFS:
    ...
    if not best: print(f"{ct:20s}   无四项达标点"); continue
    p_,r_,per,combo,fire,bpi = best
    print(f"{ct:20s} {p_:7.3f} {r_:7.3f} {per[0]:6.3f} {per[1]:6.3f} {per[2]:6.3f} "
          f"{fire:5.0%} {bpi:7.2f}")
    rows.append({"conf_type":ct,"precision":p_,"fire":fire,"bpi":bpi,"thr":combo})
if rows:
    b=min(rows,key=lambda r:(round(r["bpi"],4),-r["precision"]))
    cur=[r for r in rows if r["conf_type"]=="max"][0]
    print(f"\n现配置 max: {cur['bpi']:.2f} 箱/张 (P {cur['precision']:.3f})")
    print(f"最优 {b['conf_type']}: {b['bpi']:.2f} 箱/张 (P {b['precision']:.3f})")
    print("-> " + ("与现配置相同,无需改动" if b["conf_type"]=="max" else "须再经留出验证"))
Path("/workspace/exp_cb/conf_type_regate.json").write_text(json.dumps(rows, indent=2))
