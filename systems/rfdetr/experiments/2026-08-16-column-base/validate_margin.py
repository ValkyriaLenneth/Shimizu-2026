...
def members(device, paths, sizes, margin):
...
def main():
    ...
    for tag, single in (("m010", 0.10), ("m005", 0.05)):
    ...
    print(f"全量: 外扩0.10 {prec(tabs["m010"], full):.4f}  外扩0.05 {prec(tabs["m005"], full):.4f}")
    print(f"健全图 箱/张: 外扩0.10 {snd["m010"].mean():.3f}  外扩0.05 {snd["m005"].mean():.3f}")

    rng = np.random.default_rng(20260816)
    dp, db = [], []
    for _ in range(NBOOT):
        i1 = rng.integers(0, len(names), len(names))
        dp.append(prec(tabs["m005"], i1) - prec(tabs["m010"], i1))
        i2 = rng.integers(0, len(snd["m010"]), len(snd["m010"]))
        db.append(snd["m005"][i2].mean() - snd["m010"][i2].mean())
    for lab, d, unit in (("precision", np.array(dp), ""), ("健全 箱/张", np.array(db), "")):
        lo, hi = np.percentile(d, [2.5, 97.5])
        print(f"配对自助 {lab:12s} 均值 {d.mean():+.4f}{unit}  95% [{lo:+.4f}, {hi:+.4f}]  "
              f"外扩0.05更好 {np.mean(d < 0) if "箱" in lab else np.mean(d > 0):.1%}")
    ...
    for _ in range(NSPLIT):
        ...
        ws, wu = best_thr(tabs["m005"][2], a), best_thr(tabs["m010"][2], a)
        if ws and wu:
            d.append(prec_at(tabs["m005"][2], b, ws[1]) - prec_at(tabs["m010"][2], b, wu[1]))
    ...
    Path("/workspace/exp_cb/margin_check.json").write_text(json.dumps(
